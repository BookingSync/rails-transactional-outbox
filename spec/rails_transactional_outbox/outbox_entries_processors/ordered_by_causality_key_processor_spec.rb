...
RSpec.describe RailsTransactionalOutbox::OutboxEntriesProcessors::OrderedByCausalityKeyProcessor, :freeze_time do
  describe "#call" do
    ...
    describe "when there are some outbox entries to be processed" do
      ...
      context "when success" do
        ...
        context "when locked for one of the causality keys" do
          let(:lock_client) { Redlock::Client.new([ENV.fetch("REDIS_URL")]) }

          before do
            RailsTransactionalOutbox.configure do |config|
              config.lock_client = lock_client
            end
          end

          around do |example|
            lock_client.lock("#{RailsTransactionalOutbox}-causality_key_1", 10_000) do
              example.run
            end
          end

          it "marks as processed only the record that was actually processed as the other one as blocked by locker" do
            expect do
              call
          ...
          it "handles the actual processing via a processor" do
            expect do
              call
            end.to avoid_changing { outbox_record_1.reload.context }
              .and change { outbox_record_2.reload.context }.from("").to("updated_from_processor")
          end

          it "returns the processed records" do
            expect(call).to eq([outbox_record_2])
          end
          ...
          context "when the block is passed" do
            ...
            it "yields the block upon processing" do
              expect do
                call
              end.to avoid_changing { outbox_record_1.reload.updated_at }
                .and change { outbox_record_2.reload.updated_at }.to(10.days.from_now)
            end
          end
        end
